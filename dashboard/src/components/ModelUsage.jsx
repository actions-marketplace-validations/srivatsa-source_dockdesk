...
export default function ModelUsage({ modelUsage, dualModel, modelsUsed, availableModels, latestFiles = [] }) {
  const hasData = modelUsage && Object.keys(modelUsage).length > 0
  ...
  const entries = hasData ? Object.entries(modelUsage).sort((a, b) => b[1] - a[1]) : []
  const maxCount = Math.max(...entries.map(([, c]) => c), 1)

  // Compute diversity score
  const totalModels = modelsUsed?.length || entries.length || 0
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-surface-700 rounded-lg p-3 border border-white/5">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="text-[10px] text-info font-semibold uppercase">Code Model</span>
              </div>
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center space-x-1.5">
                      {isReasoning ? (
                        ...
                        <Code size={12} className="text-info" />
                      )}
      ...
      {rotationActive && latestFiles.length > 0 && (
        <div className="mt-5 pt-4 border-t border-white/5">
          ...
          <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1">
            {latestFiles.slice(0, 25).map((f) => (
              <div key={`${f.file}-${f.code_model}`} className="flex items-center justify-between text-[11px] bg-surface-700/70 border border-white/5 rounded-md px-2 py-1.5">
                <span className="text-slate-300 font-mono truncate pr-2" title={f.file}>{f.file}</span>
                <span className="text-info font-mono whitespace-nowrap">{f.code_model || '—'}</span>
              </div>
            ))}
          </div>
        </div>
