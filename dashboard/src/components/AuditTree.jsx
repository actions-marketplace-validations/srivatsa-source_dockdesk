...
const STATUS_ICONS = {
  ...
  ERROR: { icon: AlertTriangle, className: 'text-amber-400' },
...
function RiskPills({ counts }) {
  ...
  return (
    <div className="flex items-center space-x-1.5 ml-2">
      {counts.HIGH > 0 && (
        <span className="inline-flex items-center space-x-1 bg-red-500/15 text-red-400 text-[10px] font-semibold px-1.5 py-0.5 rounded-full">
          <span className="w-1.5 h-1.5 rounded-full bg-red-500" />
          <span>{counts.HIGH}</span>
        </span>
      )}
      {counts.MEDIUM > 0 && (
        <span className="inline-flex items-center space-x-1 bg-amber-500/15 text-amber-400 text-[10px] font-semibold px-1.5 py-0.5 rounded-full">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
          <span>{counts.MEDIUM}</span>
        </span>
      )}
      {counts.LOW > 0 && (
        <span className="inline-flex items-center space-x-1 bg-emerald-500/15 text-emerald-400 text-[10px] font-semibold px-1.5 py-0.5 rounded-full">
...
function FileDetail({ node }) {
  ...
  return (
    <div className={`mt-2 ml-7 p-3 rounded-lg border ${risk.border} ${risk.bg} animate-slideDown`}>
      <div className="flex items-center space-x-4 text-[11px] mb-2">
        ...
        {node.code_model && (
          <span className="inline-flex items-center space-x-1 text-slate-400">
            <Cpu size={11} />
            <span className="font-mono">{node.code_model}</span>
          </span>
        )}
      </div>
      {node.summary && (
        <p className="text-xs text-slate-300 mb-2">{node.summary}</p>
      )}
      {node.fix && (
        <div className="mt-2">
          <p className="text-[10px] text-amber-400 font-semibold uppercase tracking-wider mb-1">Suggested Fix</p>
          <p className="text-xs text-slate-300 font-mono bg-black/20 rounded p-2">{node.fix}</p>
        </div>
      )}
      {node.reasoning && (
        <div className="mt-2">
          <p className="text-[10px] text-info font-semibold uppercase tracking-wider mb-1">AI Reasoning</p>
...
function TreeNode({ node, depth = 0 }) {
  ...
  if (node.type === 'dir') {
    ...
    // Determine directory health color based on risk counts
    const highRisk = node.risk_counts?.HIGH || 0
    const dirColor = highRisk > 0 ? 'text-red-400' : 'text-amber-300'

    return (
      <div className="select-none">
        <button
          onClick={() => setExpanded(!expanded)}
          className={`w-full flex items-center py-1.5 px-2 rounded-md text-left hover:bg-white/[0.04] transition-colors group`}
        ...
        {expanded && hasChildren && (
          <div className="tree-branch animate-slideDown">
            {node.children
              .sort((a, b) => {
                // Directories first, then files; HIGH risk first
  ...
  return (
    <div>
      ...
      {detailOpen && <FileDetail node={node} />}
    </div>
  )
}
...
export default function AuditTree({ tree, files }) {
  // If we have a tree from backend, use it; otherwise build one from flat files
  ...
  if (!treeData || (!treeData.children?.length && !files?.length)) {
    return (
      <div className="bg-surface-600 border border-white/5 rounded-xl p-6">
        ...
        <p className="text-muted text-xs">No audit data yet. Run an audit to see the file tree with results.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-surface-600 border border-white/5 rounded-xl p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          {highCount > 0 && (
            <span className="text-[11px] text-red-400 font-semibold">{highCount} HIGH</span>
          )}
          {medCount > 0 && (
            <span className="text-[11px] text-amber-400 font-semibold">{medCount} MED</span>
          )}
          {lowCount > 0 && (
            <span className="text-[11px] text-emerald-400 font-semibold">{lowCount} LOW</span>
          )}
        </div>
      </div>
      ...
      <div className="border border-white/5 rounded-lg bg-surface-700 p-2 max-h-[600px] overflow-y-auto">
...
// Helper: count total file nodes in tree
...
// Fallback: build tree from flat file list when backend doesn't provide audit_tree
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (i === parts.length - 1) {
        current.children.push({
          ...
          risk: f.risk || 'UNKNOWN',
          safe_to_push: f.safe_to_push ?? false,
          summary: f.summary || '',
          fix: '',
          reasoning: '',
          code_model: f.code_model || '',
        ...
        if (risk === 'HIGH' || risk === 'MEDIUM' || risk === 'LOW') {
          ...
          // Propagate to intermediate dirs
          let ancestor = root
          for (let j = 0; j < i; j++) {
            const ancestorChild = ancestor.children.find(
              (c) => c.type === 'dir' && c.name === parts[j]
            )
            if (ancestorChild) {
              ancestorChild.risk_counts[risk] = (ancestorChild.risk_counts[risk] || 0) + 1
              ancestor = ancestorChild
            }
          }
        }
      } else {
        ...
        if (!dirNode) {
          dirNode = { name: part, type: 'dir', children: [], risk_counts: { HIGH: 0, MEDIUM: 0, LOW: 0 } }
